...
export default function GameActivity({ scores = [] }) {
  ...
  // Use real scores, or empty array if none provided
  ...
  // Calculate statistics based on actual game data
  const calculateStats = () => {
    ...
    gameScores.forEach(score => {
      ...
      // Estimate time per game based on score (just a rough estimate)
    ...
    <div className="bg-white/5 backdrop-blur-md rounded-xl border border-white/15 overflow-hidden mb-8">
      ...
      {activeTab === 'best' && (
        <div className="p-4">
          {gameScores.length > 0 ? (
            <div className="space-y-4">
              {/* Get unique games and their best scores */}
              {Object.entries(
          ...
          {gameScores.length > 0 ? (
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* Progress Over Time - simply show recent scores */}
                <div className="bg-white/5 p-4 rounded-lg">
